...
func run() error {
	var (
		...
		profilePath        = flag.String("profile", defaultProfilePath(), "x-twitter-cli Chromium profile")
		authenticationPath = flag.String("authentication", defaultAuthenticationPath(), "x-twitter-cli authentication state")
	...
	if info, err := os.Stat(*profilePath); err != nil || !info.IsDir() {
		return fmt.Errorf("x-twitter-cli application profile does not exist: %s; authenticate it first with: make auth-browser", *profilePath)
	}
	authentication, err := state.LoadAuthentication(*authenticationPath)
	if err != nil {
		return fmt.Errorf("load x-twitter-cli authentication state: %w", err)
	}
	...
	if err := page.SetCookies(authenticationCookies); err != nil {
		return fmt.Errorf("apply x-twitter-cli authentication state: %w", err)
	}
...
func requiredAuthenticationCookies(cookies []app.AuthenticationCookie) ([]*proto.NetworkCookieParam, error) {
	...
	if required["auth_token"] == "" || required["ct0"] == "" {
		return nil, errors.New("x-twitter-cli authentication state must contain auth_token and ct0 cookies; run make test-live to refresh it")
	}
...
func defaultProfilePath() string {
	...
	return filepath.Join(stateRoot, "x-twitter-cli", "chromium-profile")
}
...
func defaultAuthenticationPath() string {
	...
	return filepath.Join(configRoot, "x-twitter-cli", "authentication.json")
}
