...
**Application profile**:
The isolated browser profile owned by `x-twitter-cli3` for one X identity. It is distinct from the user's everyday browser profile.
...
**Source payload**:
Data returned by X before `x-twitter-cli3` defines any stable record mapping. It is evidence for later output design and carries no field-level compatibility promise.
