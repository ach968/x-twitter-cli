...
- Conversation frontier: empty pending runnable evidence
- Later questions: blocked on representative contract captures and source payloads
...
No further design question can be answered responsibly until a runnable vertical slice supplies evidence. The next phase must:

- Install and launch the Rod-managed Chromium revision through the explicit setup path.
- Establish authentication in the application profile and reuse it in a later headless launch.
- Execute both `HomeTimeline` and `SearchTimeline` through direct HTTP while preserving one operation-level interface; initialize SearchTimeline's transaction ID generator from current X frontend data.
- Preserve sanitized representative source payloads and failures for the next design round.

## Questions blocked on evidence

- Whether normalization is useful and, if so, which fields, pagination behavior, and truncation rules belong in the stable output.
- Which browser navigation reliably elicits each required operation contract.
- Which upstream failures can be classified with high confidence from observed responses.
