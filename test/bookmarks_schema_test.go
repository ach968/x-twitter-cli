...
func TestBookmarksGoldenPagesConformToSchema(t *testing.T) {
	...
	for _, resource := range []struct {
		...
		{uri: "urn:x-twitter-cli:schema:bookmarks-page", path: filepath.Join("..", "docs", "bookmarks-page.schema.json")},
	} {
	...
	contract, err := compiler.Compile("urn:x-twitter-cli:schema:bookmarks-page")
	if err != nil {
		t.Fatal(err)
	}
